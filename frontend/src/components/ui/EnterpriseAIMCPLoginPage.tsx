...
type Tenant = "esri-saudi" | "smart-city" | "platform-lab" | "demo";
...
const tenants: Array<{ id: Tenant; name: string; description: string; badge: string }> = [
...
const securitySignals: SecuritySignal[] = [
  { label: "SSO", value: "Enabled", tone: "green" },
  { label: "MFA", value: "Required", tone: "blue" },
  { label: "Vault", value: "Connected", tone: "violet" },
  { label: "Policy", value: "OPA Active", tone: "amber" },
];

const accessCards = [
  {
    title: "Agent Studio",
    description: "Create, debug, deploy, copy, and govern enterprise AI agents.",
    icon: "🤖",
  },
  {
    title: "MCP Studio",
    description: "Build secure MCP servers for ArcGIS, databases, cloud, files, and automation.",
    icon: "🧩",
  },
  {
    title: "Workflow Builder",
    description: "Draw visual workflows with approvals, policies, templates, and runtime traces.",
    icon: "🔁",
  },
];
    ...
    <section className="relative hidden min-h-screen overflow-hidden border-r border-slate-800 bg-slate-950 p-8 lg:flex lg:w-[52%] xl:w-[56%]">
      ...
      <div className="relative z-10 flex w-full flex-col justify-between">
        ...
        <div className="mx-auto max-w-3xl py-16">
          ...
          <h1 className="text-5xl font-semibold tracking-tight text-white xl:text-7xl">
...
function TenantSelector({ tenant, setTenant }: { tenant: Tenant; setTenant: (tenant: Tenant) => void }) {
  return (
    <div>
...
function PasswordLoginForm() {
  const [email, setEmail] = useState("mohamed847433@gmail.com");
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="flex items-center justify-between gap-3">
        ...
        <Badge tone="green">MFA required</Badge>
      </div>
      <button className="w-full rounded-2xl bg-gradient-to-r from-cyan-400 to-violet-500 px-5 py-3.5 text-sm font-semibold text-white shadow-lg shadow-cyan-500/20 transition hover:scale-[1.01]">
        Sign in securely
      </button>
      {submitted && (
        <div className="rounded-2xl border border-cyan-500/30 bg-cyan-500/10 p-4 text-sm leading-6 text-cyan-200">
          Demo action: authentication request prepared for <strong>{email || "selected user"}</strong>. Connect this form to Keycloak/OAuth in production.
        </div>
      )}
    </form>
  );
...
function LoginCard() {
  ...
  const [tenant, setTenant] = useState<Tenant>("esri-saudi");
...
export default function EnterpriseAIMCPLoginPage() {
  return (
